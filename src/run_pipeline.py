...
# Keep NumPy/OpenBLAS from allocating a thread pool in every pipeline process.
...
def run_pipeline():
    ...
    with concurrent.futures.ThreadPoolExecutor() as executor:
        ...
        future_record = executor.submit(
            rl.record_eeg_stream, "lsl_eeg_recording_raw.fif", stop_flag)
        time.sleep(2)
...
def run_baseline():
    ...
    processed_output_path_baseline = de.process_eeg_file(
        raw_idle_eeg_path, "datafiles/baseline.csv")
    ...
    with concurrent.futures.ThreadPoolExecutor() as executor:
        ...
        future_active = executor.submit(
            rl.record_eeg_stream, "baseline_active_raw.fif", stop_flag)
    ...
    processed_output_path = de.process_eeg_file(
        raw_active_eeg_path, "datafiles/baseline_active_processed.csv")
    se.sync_game_to_eeg(raw_active_eeg_path, processed_output_path,
                        game_log_filepath, "datafiles/synced_baseline.csv")
...
def run_baseline_real():
    ...
    with concurrent.futures.ThreadPoolExecutor() as executor:
        ...
        future_idle = executor.submit(
            rl.record_eeg_stream, "baseline_raw.fif", stop_flag)
