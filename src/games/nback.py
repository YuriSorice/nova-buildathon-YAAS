...
class NBackGame:
    ...
    def __init__(self, config, stim=1.75, game_length=15, num_match=3, log_filename="game_session_log.csv"):
        ...
        self.feedback_states = {"color": "NEUTRAL",
                                "spatial": "NEUTRAL", "audio": "NEUTRAL"}
        self.turn_counter = 0

        self.session_sequence = self.generate_sequence(
            self.GAME_LENGTH, self.MATCH_COUNT)
        self.log_filename = log_filename
        with open(self.log_filename, "w", newline="") as file:
            writer = csv.writer(file)
            writer.writerow(
                ["timestamp", "modality", "action", "performance_state"])
    ...
    def save_state(self):
        ...
        # tbr_df = ad.calculate_tbr(
        #     df, channels=['Fz', 'C3', 'C4', 'Cz', 'Pz', 'PO7', 'PO8', 'Oz'])
        # tbr_dict = tbr_df.to_dict()
        # tar_df = ad.calculate_tar(df, channels1=['Fz', 'C3', 'C4', 'Cz'], channels2=[
        #                           'PO7', 'PO8', 'Oz', 'Pz'])
        # tar_dict = tar_df.to_dict()
        # baseline_tar_df = ad.calculate_tar(
        #     baseline_df, channels=['Fz', 'C3', 'C4', 'Cz', 'Pz', 'PO7', 'PO8', 'Oz'])
        # baseline_tar_dict = baseline_tar_df.to_dict()
        # baseline_beta_df = baseline_df[['Fz_Beta', 'C3_Beta', 'C4_Beta',
        #                                 'Cz_Beta', 'Pz_Beta', 'PO7_Beta', 'PO8_Beta', 'Oz_Beta']].mean()
    ...
    def draw_grid(self):
        ...
        # horizontal lines
        pg.draw.line(self.screen, line_color, (start_x, start_y + self.CELL_SIZE),
                     (start_x + grid_width, start_y + self.CELL_SIZE), thickness)
        pg.draw.line(self.screen, line_color, (start_x, start_y + self.CELL_SIZE * 2),
                     (start_x + grid_width, start_y + self.CELL_SIZE * 2), thickness)

        # vertical lines
        pg.draw.line(self.screen, line_color, (start_x + self.CELL_SIZE, start_y),
                     (start_x + self.CELL_SIZE, start_y + grid_width), thickness)
        pg.draw.line(self.screen, line_color, (start_x + self.CELL_SIZE * 2, start_y),
                     (start_x + self.CELL_SIZE * 2, start_y + grid_width), thickness)

    def draw_color(self, color, coords):
        """Fills a specific cell with a circle of a given color"""
        start_x = int(self.GRID_CENTER_X - self.CELL_SIZE +
                      (coords[0] * self.CELL_SIZE))
        start_y = int(self.GRID_CENTER_Y - self.CELL_SIZE +
    ...
    def draw_legend(self):
        ...
        if self.cfg["use_color"]:
            active_settings.append(("COLOR [F]", "color"))
        if self.cfg["use_spatial"]:
            active_settings.append(("SPATIAL [J]", "spatial"))
        if self.cfg["use_audio"]:
            active_settings.append(("AUDIO [SPACE]", "audio"))
        ...
        spacing = 20
        total_width = len(active_settings) * box_width + \
            (1 - len(active_settings)) * spacing
        ...
        for i, (label, mod_key) in enumerate(active_settings):
            ...
            pg.draw.rect(self.screen, state_color,
                         (bx, by, box_width, box_height), border_radius=6)
            pg.draw.rect(self.screen, self.COLORS["DARK_GREY"], (
                bx, by, box_width, box_height), 2, border_radius=6)

            txt_surface = self.font.render(label, True, self.COLORS["BLACK"])
            txt_rect = txt_surface.get_rect(
                center=(bx+box_width // 2, by + box_height // 2))
    ...
    def generate_sequence(self, length, match_count):
        ...
        sequence = []
        color_targets = set(random.sample(
            range(n_back, length), match_count)) if self.cfg["use_color"] else set()
        spatial_targets = set(random.sample(
            range(n_back, length), match_count)) if self.cfg["use_spatial"] else set()
        audio_targets = set(random.sample(
            range(n_back, length), match_count)) if self.cfg["use_audio"] else set()
    ...
    def run(self):
        ...
        while running:
            ...
            if elapsed >= self.STIMULUS_DURATION:
                ...
                for mod in ["color", "spatial", "audio"]:
                    ...
                    if self.cfg[config_key] and self.is_target[mod] and not self.responses[mod]:
                        ...
                        print(
                            f"[{time.time()}] {mod.upper()} INATTENTION ERROR - Missed target")
                ...
                if self.turn_counter >= self.GAME_LENGTH:
                    ...
                    self.responses = {"color": False,
                                      "spatial": False, "audio": False}
                    self.feedback_states = {
                        "color": "NEUTRAL", "spatial": "NEUTRAL", "audio": "NEUTRAL"}

                    # Calculate new n-back targets
                    if self.turn_counter >= self.cfg["n_back"]:
                        target_stimulus = self.session_sequence[self.turn_counter -
                                                                self.cfg["n_back"]]
                        self.is_target = self.get_target_matches(
                            current_stimulus, target_stimulus)
                    else:
                        is_target = {"color": False,
                                     "spatial": False, "audio": False}
            ...
            if current_stimulus:
                color_val = self.COLORS[current_stimulus["color"]
                                        ] if self.cfg["use_color"] else self.COLORS["BLUE"]
