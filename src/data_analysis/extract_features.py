...
raw_path = script_dir.parent / "datafiles" / \
...
for epoch_idx, epoch_data in enumerate(data):
    freqs, psd = welch(epoch_data, fs=sfreq, nperseg=min(
        int(sfreq * 2), epoch_data.shape[1]))
# getting bandpower for different frequencies
    ...
    # how hard you're focusing/thinking actively
    engagement = mean_beta / (mean_alpha + mean_theta + 1e-6)
    # how distracted, how idle your brain is
